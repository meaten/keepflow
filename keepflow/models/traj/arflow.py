...
class ARFlow(ModelTemplate):
    ...
    def predict(self, data_dict: Dict, return_prob=False) -> Dict:
        ...
        for k in range(self.pred_len):
            current_dec_inputs.append(torch.cat([sample, dec_pe[k][None].tile(1, sample.shape[1], 1)], dim=-1))
...
class Flow(nn.Module):
    ...
    def inverse(self, u, cond):
        ...
        if self.scale is not None:
            x *= self.scale
            log_abs_det_jacobian += torch.log(torch.abs(self.scale).sum(dim=-1))
        return x, log_abs_det_jacobian
