...
class TrajMetrics(object):
    def __init__(self, cfg: CfgNode):
        ...
        self.pred_len = cfg.DATA.OBSERVE_LENGTH
        ...
        self.mean = dict()  # not used
        self.std = dict()
        for node_type in self.env.NodeType:
            # assume all node type use the same state
            self.state = hypers[cfg.DATA.TRAJ.PRED_STATE][node_type.name]
            mean, std = self.env.get_standardize_params(self.state, node_type.name)
            self.mean[node_type] = torch.Tensor(mean).to(self.device)
            self.std[node_type] = torch.Tensor(std).to(self.device)

            if cfg.DATA.TRAJ.PRED_STATE == 'state_p':
                std = self.env.attention_radius[(node_type.name, node_type.name)]
                self.std[node_type] = torch.Tensor([std, std]).to(self.device)
    ...
    def unstandardize(self, data_dict: Dict) -> Dict:
        std = torch.cat([self.std[nt][None] for nt in data_dict['this_node_type']])[:, None]
        
        assert 'acceleration' not in self.state.keys()
        if 'position' in self.state.keys():
            assert torch.all(data_dict['gt_st'] * std + data_dict['obs'][:, -1:, 0:2] - data_dict['gt'] < 1e-5)
        elif 'velocity' in self.state.keys():
            assert torch.all(data_dict['gt_st'] * std - data_dict['gt'] < 1e-5)
        
        data_dict[('pred', 0)] = data_dict[('pred_st', 0)] * std
        
        for k in list(data_dict.keys()):
            if k[0] == "prob_st" and type(data_dict[k]) == torch.Tensor:
                data_dict[("prob", k[1])] = data_dict[k].clone()
                data_dict[("prob", k[1])][..., :2] *= std
