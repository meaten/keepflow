...
import os
from data.MP import forward_kinematics
...
def load_data(path_to_dataset, subjects, actions, sample_rate, seq_len, input_n=10, data_mean=None, data_std=None):
            ...
            if not (subj == 5):
                for subact in [1, 2]:  # subactions
                    ...
                    if len(sampled_seq) == 0:
                        sampled_seq = [seq_sel]
                        complete_seq = [the_sequence]
                        action_list = [action] * len(seq_sel)
                    else:
                        sampled_seq.append(seq_sel)
                        complete_seq.append(the_sequence)
                        action_list.extend([action] * len(seq_sel))
            else:
                print("Reading subject {0}, action {1}, subaction {2}".format(
                    subj, action, 1))
                filename = '{0}/S{1}/{2}_{3}.txt'.format(
                ...
                if len(sampled_seq) == 0:
                    sampled_seq = [np.concatenate([seq_sel1, seq_sel2], axis=0)]
                    complete_seq = [np.concatenate([the_sequence1, the_sequence2], axis=0)]
                    action_list = [action] * (len(seq_sel1) + len(seq_sel2))
                else:
                    sampled_seq.append(np.concatenate([seq_sel1, seq_sel2], axis=0))
                    complete_seq.append(np.concatenate([the_sequence1, the_sequence2], axis=0))
                    action_list.extend([action] * (len(seq_sel1) + len(seq_sel2)))
    ...
    return sampled_seq, dimensions_to_ignore, dimensions_to_use, data_mean, data_std, action_list
        ...
        for action_idx in np.arange(len(actions)):
            ...
            if not (subj == 5):
                ...
                if len(sampled_seq) == 0:
                    ...
                    action_list = [action] * len(sampled_seq)
                else:
                    sampled_seq = np.concatenate(
